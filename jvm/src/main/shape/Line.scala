...
object Line {
  def parseLine(text: String): Line = {
    ...
    val line: Line = new Line(new Point(x1, y1), new Point(x2, y2), java.awt.Color.decode(color))
    line.setMarked(b1)
    line
  }
}
...
@SerialVersionUID(0L)
class Line(color: Color) extends Element(color) with Cloneable {
  ...
  def contains(p: Point): Boolean = {
    val check: Line2D.Double = new Line2D.Double(start, end)
    check.ptSegDist(p) < 3
  }
  ...
  override def toString: String = {
    "Line " + c.getRGB + " " + marked + " " + start.x + " " + start.y + " " + end.x + " " + end.y
  }

  def toReadableString: String = {
    "Line with color " + c + " and bounds " + getBounds
  }
