...
@SerialVersionUID(0L)
object LinkShape {
  def parseShapes(shapes: Array[String], version: String): JList[Shape] = {
    var index: Int = 0
    val ret: JList[Shape] = new JArrayList[Shape]
    var shape: LinkShape = null
    while ((shapes.length > index) && (0 == VectorShape.getString(shapes, index).length)) {
      index += 1
    }
  ...
  def parseShape(shapes: Array[String], version: String, shape: LinkShape, initialIndex: Int): Int = {
    var index = initialIndex
    var line: LinkLine = null
    shape.setName(VectorShape.getString(shapes, {
      index += 1; index - 1
    }))
    shape.curviness = VectorShape.getString(shapes, {
      index += 1; index - 1
    }).toDouble
    for (i <- 0 until 3) {
      line = new LinkLine
    ...
    val indicator: VectorShape = new VectorShape
  ...
  def getDefaultLinkDirectionShape: VectorShape = {
    ...
    result.addElement("Line -7500403 true 150 150 90 180")
    result.addElement("Line -7500403 true 150 150 210 180")
    result
  }
}

@SerialVersionUID(0L)
class LinkShape extends org.nlogo.core.Shape with Cloneable with java.io.Serializable with DrawableShape {
  private var directionIndicator: VectorShape = LinkShape.getDefaultLinkDirectionShape
  private var curviness: Double = .0
  ...
  def getDashes(index: Int): Array[Float] = lines(index).getDashes

  def curviness(curviness: Double): Unit = {
    this.curviness = curviness
  }

  def dashinessString(index: Int): String = lines(index).dashinessString

  def setDashes(index: Int, str: String) = lines(index).setDashes(str)

  def isTooSimpleToPaint: Boolean =
    !lines(0).isVisible && !lines(2).isVisible && curviness == 0 && lines(1).isStraightPlainLine

  def getDirectionIndicator: Shape = directionIndicator

  def setDirectionIndicator(shape: VectorShape) = {
  ...
  def paint(g: GraphicsInterface, color: Color,
            ...
            isDirected: Boolean): Unit = {
    for (line <- lines if line.isVisible)  {
  ...
  def getDirectionIndicatorTransform(x1: Double, y1: Double, x2: Double, y2: Double, linkLength: Double, destSize: Double, link: Link, cellSize: Double, size: Double): Array[Double] = {
    if (curviness == 0) {
      Array[Double](
        link.heading,
      ...
      val trans: Array[Double] = getDirectionIndicatorTransform(arc, getDestShape(arc, destSize, cellSize))
      trans(0) = -trans(0) + 180
      trans
    }
  }
  ...
  def paintDirectionIndicator(g: GraphicsInterface, color: Color,
    ...
    directionIndicator.paint(g, color, x2 + xcomp - (cellSize * scaleFactor / 2), y2 - ycomp - (cellSize * scaleFactor / 2), scaleFactor, cellSize, heading.toInt, lineThickness)
  }

  def numLines: Int = lines.count(_.isVisible)
  ...
  def getDestShape(arc: JShape, size: Double, cellSize: Double): JShape = {
  ...
  def getDirectionIndicatorTransform(arc: JShape, dest: JShape): Array[Double] = {
    ...
    while (!i.isDone) {
      {
        ...
        if (ret == PathIterator.SEG_MOVETO) {
          ...
          if (dest.contains(p1) && !dest.contains(p2)) {
            return getDirectionIndicatorTransform(new Line2D.Double(p1, p2), dest)
          }
        }
      }
      i.next()
  ...
  def getDirectionIndicatorTransform(initialLine: Line2D, dest: JShape): Array[Double] = {
  ...
  protected def getLastOutsideSegment(initialLine: Line2D, dest: JShape): Line2D = {
    var line = initialLine
