...
public strictfp class ExtensionManager
  ...
  public String resolvePathAsURL(String path) {
    ...
    try {
      java.net.URL jarURL = new java.net.URL(path);
      return jarURL.toString();
    } catch (java.net.MalformedURLException ex) {
      org.nlogo.api.Exceptions.ignore(ex);
    }

    ArrayList<String> potentialJarPaths = new ArrayList<String>();

    try {
      potentialJarPaths.add(attachModelDir(path)); // extension relative to model
    } catch (java.net.MalformedURLException ex) {
      org.nlogo.api.Exceptions.ignore(ex);
    }

    // path relative to `pwd`/extensions
    potentialJarPaths.add("extensions" + java.io.File.separator + path);

    // path relative to property, defaults to ~/
    // this exists in part to support running the tests from any directory
    // not just the directory containing the extensions directory RG 2/24/2015
    String home = System.getProperty("user.home");
    String netLogoExtensionDir = (System.getProperty("netlogo.extensions.dir", home) + java.io.File.separator + path);
    potentialJarPaths.add(netLogoExtensionDir);

    for (String potentialPath : potentialJarPaths) {
      try {
        java.io.File jarFile = new java.io.File(potentialPath);
        if (jarFile.exists()) {
