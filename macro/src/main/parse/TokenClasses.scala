...
object TokenClasses {
  ...
  def compiledReporters[T](packagePrefix: String): Map[String, () => T] =
    macro _compileReporters[T]
  def compiledCommands[T](packagePrefix: String): Map[String, () => T] =
    macro _compileCommands[T]

  def packageConstructors[T](packagePrefix: String): Map[String, (String => T)] =
    macro _packageConstructors[T]

  def _compileReporters[T: c.WeakTypeTag](c: BlackBoxContext)(packagePrefix: c.Tree): c.Tree = {
    import c.universe._
    packagePrefix match {
  ...
  def _compileCommands[T: c.WeakTypeTag](c: BlackBoxContext)(packagePrefix: c.Tree): c.Tree = {
    import c.universe._
    packagePrefix match {
      case q"${packageName: String}" =>
        val mapElems = FileEntries.collect {
  ...
  def _packageConstructors[T: c.WeakTypeTag](c: BlackBoxContext)(
