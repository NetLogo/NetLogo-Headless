...
object ShapeParser {
  ...
  private def parseNewlineSeparatedGroups[A](lines: Seq[String], f: List[String] => A): Seq[A] =
    lines.foldLeft(List[List[String]]()) {
      case ((Nil)::otherShapes, l: String) if l == "" => List[String](l)::otherShapes
      case (otherShapes, l: String) if l == "" => List[String]()::otherShapes
      case (currentShape::otherShapes, l: String) => (currentShape :+ l)::otherShapes
      case (Nil, l: String) => List(l)::Nil
    }.filter(_ != List()).reverse.map(f)
  ...
  private def parseLinkLine(line: String): LinkLine = {
    line.split(' ').toList match {
      case xcor::"1"::dashChoices if LinkLine.dashChoices.contains(dashChoices.map(_.toFloat).toArray) =>
  ...
  private def parseElement(s: String): Element = {
    val shapeName::sections = s.split(' ').toList
    try {
      (shapeName, sections) match {
  ...
  object LinkLine {
    // this should maybe be a Set instead of an array
