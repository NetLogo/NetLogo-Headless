...
class ShapeParserTests extends FunSuite {
  ...
  def shapeString(name: String, rotatable: String, editableColorIndex: String, elements: String*): Seq[String] = {
  ...
  test("ShapeParser parses Line elements") {
    assertParsesTo(
      shapeString("line", "false", "0", "Line -7500403 true 150 0 150 300"),
  ...
  test("ShapeParser parses Rectangle elements") {
    assertParsesTo(
      shapeString("rectangle", "false", "0", "Rectangle -7500403 true true 0 0 150 150"),
  ...
  test("ShapeParser parses Polygon elements") {
    assertParsesTo(
      shapeString("poly", "false", "0", "Polygon -16777216 true false 238 112 252 141 219 141 218 112"),
  ...
  test("ShapeParser raises an exception on invalid shape values") {
    intercept[IllegalStateException] {
      ShapeParser.parseVectorShapes(shapeString("invalidLine", "false", "0", "Line -7500403 abasoniqeworn 150 0 150 300"))
    }
  }

  test("ShapeParser raises an exception on invalid number of shape fields") {
    intercept[IllegalStateException] {
      ShapeParser.parseVectorShapes(shapeString("invalidshapecount", "false", "0", "Line -7500403 true 150 0 25"))
    }
  }

  test("ShapeParser raises an exception on invalid shape names") {
    intercept[IllegalStateException] {
      ShapeParser.parseVectorShapes(shapeString("invalidshapetype", "false", "0", "Rhombus -7500403 true 150 0 25 25"))
    }
  }

  test("shapeParser parses multiple vector shapes") {
    val twoRects = shapeString("box", "true", "0", "Rectangle -7500403 true false 0 0 100 100") ++ Seq("") ++
       shapeString("box2", "true", "0", "Rectangle -7500403 true false 0 0 200 200")
  ...
  def linkShapeText(name: String = "default",
  ...
  test("ShapeParser parses a valid link shape into a LinkShape") {
    assertResult(Seq(LinkShape("default", 0.0, defaultLinkShapes, defaultDirectionIndicator)))(
      ShapeParser.parseLinkShapes(linkShapeText("default")))
  }
  ...
  test("shapeParser raises an exception on invalid linkLines") {
    intercept[IllegalStateException] {
      ShapeParser.parseLinkShapes(linkShapeText(name = "foobar", linkLines = Seq("foo ooo bar", "-,.,-", "\u1234")))
    }
  }

  test("shapeParser parses multiple link shapes") {
    assertResult(
      Seq(LinkShape("default", 0.0, defaultLinkShapes, defaultDirectionIndicator),
        LinkShape("default2", 0.0, defaultLinkShapes, defaultDirectionIndicator)))(
        ShapeParser.parseLinkShapes(linkShapeText("default") ++ Seq("") ++ linkShapeText("default2")))
    }

  test("shapeParser parses a link with no name if it's the second link") {
    assertResult(
      Seq(LinkShape("default", 0.0, defaultLinkShapes, defaultDirectionIndicator),
        LinkShape("", 0.0, defaultLinkShapes, defaultDirectionIndicator)))(
      ShapeParser.parseLinkShapes(linkShapeText() ++ Seq("") ++ linkShapeText(name = ""))
    )
  }

  test("shapeParser parses link lines to have any of the permitted dash values") {
    val dashedLinkLines = linkShapeText(name = "dashedLines",
  ...
  test("shapeParser parses invalid link line values to be solid lines") {
    val invalidLinkLines = linkShapeText(name = "dashedLines",
  ...
  test("shapeParser parses invisible link lines to be dashed invisibly") {
    val invisibleLinkLines = linkShapeText(name = "invisibleLines",
