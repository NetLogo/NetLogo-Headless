...
import org.nlogo.api.{FrontEndInterface, Program, ParserServices}
...
class DefaultParserServices(parser: FrontEndInterface) extends ParserServices {
  ...
  def isReporter(s: String) =
    parser.isReporter(s, Program.empty(), FrontEndInterface.NoProcedures,
